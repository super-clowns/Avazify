...
export default function SettingsPage() {
  ...
  const handlePlan = () => {
    ...
    pushToast('تغییر اشتراک در محیط آزمایشی با موفقیت انجام شد.', 'success');
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="تنظیمات" title="تنظیمات برنامه و حساب" description="اعلانات، صدا، زبان، اشتراک و داده‌های آزمایشی حساب را مدیریت کنید." />
      ...
      <div className="settings-layout">
        <div className="settings-main">
          ...
          <section className="content-card settings-section danger-section">
            ...
            <div className="danger-action-row"><div><strong>بازنشانی داده‌های آزمایشی</strong><p>تمام تغییرات Local Storage حذف و پروژه به وضعیت اولیه بازمی‌گردد.</p></div><button type="button" className="button button-secondary" onClick={() => setResetOpen(true)}>بازنشانی داده‌ها</button></div>
        ...
        <aside className="settings-aside">
          ...
          <section className="storage-card"><span><Icon name="info" /></span><div><strong>ذخیره‌سازی فاز اول</strong><p>حساب‌ها، تنظیمات، آثار و تعاملات در Local Storage مرورگر ذخیره می‌شوند.</p></div></section>
        </aside>
      </div>

      <Modal open={subscriptionOpen} title="انتخاب اشتراک" description="در فاز اول، پرداخت به‌صورت ماک شبیه‌سازی می‌شود." onClose={() => setSubscriptionOpen(false)} size="large" footer={<><button type="button" className="button button-ghost" onClick={() => setSubscriptionOpen(false)}>انصراف</button><button type="button" className="button button-primary" onClick={handlePlan}>تأیید و پرداخت آزمایشی</button></>}>
        ...
        {selectedPlan !== 'free' ? (
          <div className="checkout-box">
            <label><span>مدت اشتراک</span><select value={duration} onChange={(event) => setDuration(Number(event.target.value))}><option value={1}>۱ ماهه</option><option value={3}>۳ ماهه - ۳٪ تخفیف</option><option value={6}>۶ ماهه - ۸٪ تخفیف</option><option value={12}>۱۲ ماهه - ۱۵٪ تخفیف</option></select></label>
            <div><span>مبلغ قابل پرداخت</span><strong>{payable.toLocaleString('fa-IR')} تومان</strong></div>
          </div>
        ) : null}
      </Modal>

      <Modal open={resetOpen} title="بازنشانی داده‌های محلی" onClose={() => setResetOpen(false)} size="small" footer={<><button type="button" className="button button-ghost" onClick={() => setResetOpen(false)}>انصراف</button><button type="button" className="button button-danger" onClick={handleReset}>تأیید بازنشانی</button></>}>
      ...
      <Modal open={deleteOpen} title="حذف حساب کاربری" onClose={() => setDeleteOpen(false)} size="small" footer={<><button type="button" className="button button-ghost" onClick={() => setDeleteOpen(false)}>انصراف</button><button type="button" className="button button-danger" onClick={handleDelete}>حذف دائمی حساب</button></>}>
        <div className="confirm-content"><span className="confirm-icon danger"><Icon name="trash" size={28} /></span><p>حساب «{currentUser.displayName}» و تمام داده‌های وابسته به آن از محیط آزمایشی حذف می‌شود.</p></div>
      </Modal>
    </div>
  );
}
