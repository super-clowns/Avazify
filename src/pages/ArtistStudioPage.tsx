...
function TrackFormModal({ open, track, onClose }: { open: boolean; track: Track | null; onClose: () => void }) {
  ...
  const handleCover = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => setForm((previous) => ({ ...previous, cover: String(reader.result) }));
      ...
      <form id="track-form" className="studio-form" onSubmit={handleSubmit}>
        ...
        <div className="studio-form-fields form-grid two-columns">
          ...
          <label className="audio-upload form-span-two">
            <input type="file" accept="audio/mpeg,audio/wav,audio/flac,.mp3,.wav,.flac" onChange={(event) => setForm({ ...form, audioFileName: event.target.files?.[0]?.name ?? '' })} />
            <span className="upload-field-icon"><Icon name="upload" /></span>
            <span><strong>{form.audioFileName || 'فایل صوتی اثر'}</strong><small>فرمت‌های MP3، WAV و FLAC؛ در فاز اول فایل دموی محلی پخش می‌شود.</small></span>
...
export default function ArtistStudioPage() {
  ...
  const confirmDelete = () => {
    ...
    deleteArtistTrack(deletingTrack.id);
    pushToast('اثر از فهرست منتشرشده حذف شد.', 'success');
