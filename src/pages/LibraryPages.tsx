...
function PlaylistFormModal({ open, title, initialName = '', initialDescription = '', onClose, onSubmit }: { open: boolean; title: string; initialName?: string; initialDescription?: string; onClose: () => void; onSubmit: (name: string, description: string) => void }) {
...
export function PlaylistsPage() {
  ...
  const create = (name: string, description: string) => {
    const result = createPlaylist(name, description);
  ...
  const rename = (name: string) => {
    ...
    pushToast('نام پلی‌لیست تغییر کرد.', 'success');
    setEditing(null);
  };
  ...
  const remove = () => {
    ...
    setDeleting(null);
    pushToast('پلی‌لیست حذف شد.', 'success');
  };
