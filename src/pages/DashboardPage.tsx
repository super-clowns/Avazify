...
function RequestReviewModal({ request, onClose }: { request: ArtistRequest | null; onClose: () => void }) {
  ...
  const approve = () => {
    if (!request) return;
    reviewArtistRequest(request.id, true);
    pushToast('حساب هنرمند تأیید و اعلان نتیجه ارسال شد.', 'success');
    onClose();
  };
  ...
  const reject = () => {
    ...
    reviewArtistRequest(request.id, false, reason.trim());
    pushToast('درخواست رد و علت برای هنرمند ارسال شد.', 'success');
    onClose();
  };
...
function TicketChatModal({ ticket, onClose }: { ticket: SupportTicket | null; onClose: () => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    setReply('');
    pushToast('پاسخ پشتیبانی ثبت شد.', 'success');
  };
    ...
    <Modal open={ticket !== null} title={ticket ? `${ticket.id} · ${ticket.subject}` : 'تیکت'} description={ticket ? `${ticket.userName} · وضعیت ${ticketStatusLabel[ticket.status]}` : undefined} onClose={onClose} size="large">
      {ticket ? (
        <div className="ticket-chat">
          ...
          <div className="ticket-status-actions"><span>تغییر وضعیت:</span>{(['open', 'answered', 'closed'] as TicketStatus[]).map((status) => <button type="button" className={ticket.status === status ? 'active' : ''} key={status} onClick={() => setTicketStatus(ticket.id, status)}>{ticketStatusLabel[status]}</button>)}</div>
        </div>
      ) : null}
    </Modal>
  );
...
export default function DashboardPage() {
  ...
  const subscriptionCounts = useMemo(() => ({
    free: listenerUsers.filter((user) => user.subscription === 'free').length,
    silver: listenerUsers.filter((user) => user.subscription === 'silver').length,
    gold: listenerUsers.filter((user) => user.subscription === 'gold').length,
  }), [listenerUsers]);
  const totalSubscriptions = Math.max(listenerUsers.length, 1);
  ...
  const monthlyRevenue = subscriptionCounts.silver * data.prices.silver + subscriptionCounts.gold * data.prices.gold;
  ...
  const settle = (id: string) => {
    const result = settleFinanceRecord(id);
    pushToast(result.message, result.success ? 'success' : 'error');
  };

  const savePrices = (event: FormEvent) => {
    event.preventDefault();
    const result = updatePrices(silver, gold);
    pushToast(result.message, result.success ? 'success' : 'error');
  };
      ...
      <section className="dashboard-panel">
        ...
        <div className="dashboard-tab-content">
          ...
          {tab === 'finance' && roleIsAdmin ? (
            <div className="tab-section">
              <div className="card-heading"><div><span className="eyebrow">Monthly audit</span><h2>حسابرسی و پاداش هنرمندان</h2><p>مقادیر این فاز ماک هستند و ساختار نمایش اطلاعات مالی را شبیه‌سازی می‌کنند.</p></div></div>
          ...
          {tab === 'admin' && roleIsAdmin ? (
            <div className="admin-tools-grid">
              ...
              <section className="revenue-overview-card">
                ...
                <div><span className="eyebrow">درآمد ماه جاری</span><strong>{monthlyRevenue.toLocaleString('fa-IR')} تومان</strong><p>برآورد فروش اشتراک‌های فعال براساس قیمت‌های فعلی سامانه.</p></div>
                <span className="growth-pill"><Icon name="arrow" size={15} /> ۱۲٪ رشد</span>
              </section>
            </div>
          ) : null}
        </div>
