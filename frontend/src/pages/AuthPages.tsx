...
export function RegisterPage() {
  ...
  const [artist, setArtist] = useState({ artistName: '', email: '', password: '', confirmPassword: '', portfolioUrl: '', portfolioFiles: [] as File[] });
  ...
  const handleFiles = (event: ChangeEvent<HTMLInputElement>) => {
    setArtist((previous) => ({ ...previous, portfolioFiles: Array.from(event.target.files ?? []) }));
  };
    ...
    <AuthShell mode="register">
      <div className="auth-form-wrap register-wrap">
        <div className="auth-form-heading">
          ...
          <p>نوع حساب را انتخاب کنید؛ اطلاعات حساب به‌صورت امن در بک‌اند ذخیره می‌شود.</p>
        </div>
          ...
          {mode === 'listener' ? (
            ...
            <div className="form-grid two-columns">
              ...
              <label className="upload-field form-span-two">
                ...
                <span><strong>بارگذاری نمونه‌کار</strong><small>فایل صوتی یا PDF؛ فایل در سرور ذخیره می‌شود.</small></span>
                <span className="button button-secondary">انتخاب فایل</span>
              </label>
              {artist.portfolioFiles.length ? <div className="selected-files form-span-two">{artist.portfolioFiles.map((file) => <span key={`${file.name}-${file.lastModified}`}><Icon name="music" size={15} />{file.name}</span>)}</div> : null}
            </div>
          )}
      ...
      <Modal open={privacyOpen} title="سیاست حریم خصوصی" onClose={() => setPrivacyOpen(false)} size="small" footer={<button type="button" className="button button-primary" onClick={() => { setAcceptedPrivacy(true); setPrivacyOpen(false); }}>می‌پذیرم</button>}>
        <div className="policy-content">
          <p>اطلاعات حساب و تنظیمات در پایگاه داده بک‌اند ذخیره می‌شوند و نشست ورود با توکن JWT مدیریت می‌شود.</p>
          ...
          <p>دسترسی به اطلاعات براساس نقش و سطح اشتراک در بک‌اند کنترل می‌شود.</p>
        </div>
      </Modal>
    </AuthShell>
  );
...
export function ResetPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (password !== confirmation) return setMessage('رمز عبور و تکرار آن یکسان نیست.');
    const uid = searchParams.get('uid') ?? '';
    const token = searchParams.get('token') ?? '';
    if (!uid || !token) return setMessage('لینک بازیابی ناقص یا نامعتبر است.');
  ...
  return (
    <AuthShell mode="forgot">
      <div className="auth-form-wrap compact-auth-wrap">
        <div className="forgot-icon"><Icon name="lock" size={30} /></div>
        <div className="auth-form-heading"><span className="eyebrow">امنیت حساب</span><h2>تعیین رمز عبور جدید</h2><p>رمز تازه را وارد و تکرار کنید.</p></div>
        <form className="auth-form" onSubmit={submit}>
          <Field label="رمز عبور جدید" icon="lock"><input dir="ltr" type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></Field>
          <Field label="تکرار رمز عبور" icon="lock"><input dir="ltr" type="password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></Field>
          {message ? <div className="form-alert info"><Icon name="info" />{message}</div> : null}
          <button type="submit" className="button button-primary full">ذخیره رمز جدید</button>
        </form>
      </div>
    </AuthShell>
  );
