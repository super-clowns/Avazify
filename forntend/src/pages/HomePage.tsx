...
export default function HomePage() {
  ...
  const allOwnPlaylists = data.playlists.filter((playlist) => playlist.userId === currentUser.id);
  const ownPlaylists = [...allOwnPlaylists]
    ...
    <div className="page-stack">
      ...
      <section className="stats-grid four">
        ...
        <article className="stat-card">
          ...
          <div><small>پلی‌لیست‌های من</small><strong>{allOwnPlaylists.length.toLocaleString('fa-IR')}{playlistLimit ? ` از ${playlistLimit.toLocaleString('fa-IR')}` : ''}</strong></div>
      ...
      {data.recommendations.length ? (
        <section className="section-block recommendation-section">
          <div className="section-title-row">
            <div><span className="eyebrow">پیشنهاد هوشمند</span><h2>برای سلیقه تو</h2><p>براساس تاریخچه شنیدن، سبک‌های محبوب، هنرمندان دنبال‌شده و میزان محبوبیت آثار.</p></div>
            <span className="section-icon purple"><Icon name="sparkles" /></span>
          </div>
          <div className="media-grid four">{data.recommendations.slice(0, 4).map((track) => <TrackCard key={track.id} track={track} queue={data.recommendations} />)}</div>
        </section>
      ) : null}
      ...
      <section className="section-block">
        <div className="section-title-row">
          <div><span className="eyebrow">ادامه مسیر</span><h2>پلی‌لیست‌های اخیراً ویرایش‌شده</h2></div>
